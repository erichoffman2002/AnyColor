...
class ColorBalanceApp(QMainWindow):
    ...
    def adjust_image_size(self):
        ...
        self.status_text.append(f"Adjusting image size for {self.current_glow} effect (padding: {padding}px)...")
        self.status_text.repaint()
        
        # Create new image with padding
        new_width = self.original_size[0] + padding
        new_height = self.original_size[1] + padding
        ...
        # Update both original and adjusted images
        self.original_image = new_image.copy()
        self.adjusted_image = new_image.copy()
    ...
    def adjust_pixel(self, r, g, b, a, cr_offset, mg_offset, yb_offset, hue_offset):
    ...
    def save_image_with_transparency(self, image, file_path, transparency):
    ...
    def apply_adjustments(self):
        ...
        if letter_effects_changed or self.letter_image is None:
            ...
            if self.current_effect != "None":
                self.letter_image = self.apply_color_option(self.current_effect, self.letter_image)
        ...
        if self.current_glow != "None":
            ...
            self.apply_glow_effect(self.current_glow, final_image)
            self.adjusted_image = final_image
    ...
    def save_image(self):
        ...
        if file_path:
            if not file_path.lower().endswith('.png'):
                file_path += '.png'
            self.save_image_with_transparency(self.adjusted_image, file_path, self.transparency_checkbox.isChecked())
    ...
    def apply_color_option(self, option, image):
    ...
    def apply_glow_effect(self, effect_type, image):
        """Applies a colored glow or border effect"""
        if effect_type == "None":
            return
            
        # Store the current effect type
        ...
        self.status_text.append(f"Applying {effect_type} effect...")
        self.status_text.repaint()
        ...
        self.status_text.append("Effect applied.")
        self.status_text.repaint()
        self.update_image()
        self.update_effects_list()

        # Ensure the 'None' button stays unchecked (grey) once an effect is applied
        self.none_glow_button.setChecked(False)
    ...
    def get_gradient_colors(self, x, y, width, height):
        ...
        if gradient_type == "Linear":
            direction = self.gradient_direction.currentText()
    ...
    def adjust_size_for_glow(self):
        ...
        paste_x = padding // 2
        paste_y = padding // 2
        new_image.paste(self.adjusted_image, (paste_x, paste_y))
        self.adjusted_image = new_image.copy()
        self.status_text.append("Image size adjusted.")
        self.status_text.repaint()
